Engine.Plugins.contacts = {
	...
	init:function(){
		Engine.GUI.Sidebar.Nav.add('Contacts', 'development');
		var isInitialized = setInterval(function() {
			if(Engine.initiated){
				...
				for(var [key, plugin] of Object.entries(['organizations','leads','my_leads','my_prospects','clients','my_clients'])){
					Engine.Plugins.contacts.customize(plugin);
				}
			}
		}, 100);
	},
		...
		if(Engine.Helper.isSet(Engine.Contents,['Settings','plugins',plugin,'status']) && Engine.Contents.Settings.plugins[plugin].status){
			var isInitialized = setInterval(function() {
				if(Engine.Helper.isSet(Engine.Plugins,[plugin,'forms','create'])){
					clearInterval(isInitialized);
					Engine.Plugins[plugin].forms.create.contact = {
						0:"first_name",
						1:"middle_name",
						2:"last_name",
						3:"job_title",
	...
	load:{
		index:function(){
			Engine.Builder.card($('#pagecontent'),{ title: 'Contacts', icon: 'contacts'}, function(card){
				Engine.request('contacts','read',{
					data:{options:{ link_to:'ContactsIndex',plugin:'contacts',view:'index' }},
				},function(result) {
					var dataset = JSON.parse(result);
					if(dataset.success != undefined){
						for(var [key, value] of Object.entries(dataset.output.dom)){ Engine.Helper.set(Engine.Contents,['data','dom','contacts',value.id],value); }
						for(var [key, value] of Object.entries(dataset.output.raw)){ Engine.Helper.set(Engine.Contents,['data','raw','contacts',value.id],value); }
		...
		details:function(){
			...
			setTimeout(function() {
				...
				if(values == ''){
					Engine.request('contacts','read',{data:{id:id,key:'id'}},function(result){
						var dataset = JSON.parse(result);
						if(dataset.success != undefined){
							for(var [key, value] of Object.entries(dataset.output.dom)){ Engine.Helper.set(Engine.Contents,['data','dom','contacts',value.id],value); }
							for(var [key, value] of Object.entries(dataset.output.raw)){ Engine.Helper.set(Engine.Contents,['data','raw','contacts',value.id],value); }
							var contact = Engine.Contents.data.raw.contacts[id];
							Engine.GUI.insert(contact);
						}
					});
				} else {
					var contact = Engine.Contents.data.raw.contacts[id];
	...
	Timeline:{
		...
		object:function(dataset,layout,options = {},callback = null){
			...
			var defaults = {icon: Engine.Plugins.contacts.Timeline.icon,color: "olive"};
			...
			if(typeof dataset.id !== 'undefined'){
				...
				var checkExist = setInterval(function() {
					if(layout.timeline.find('div.time-label[data-dateus="'+dateUS+'"]').length > 0){
						clearInterval(checkExist);
						Engine.Builder.Timeline.add.filter(layout,'contacts','Contacts');
						var html = '';
						...
						if(Engine.Helper.isSet(layout,['tabs','contacts'])){
		...
		details:{
			tab:function(data,layout,options = {},callback = null){
				...
				if(!Engine.Helper.isSet(layout,['content','contacts'])){
					Engine.GUI.Layouts.details.tab(data,layout,{icon:"fas fa-address-book",text:Engine.Contents.Language["Contacts"]},function(data,layout,tab,content){
						...
						if(Engine.Auth.validate('plugin', 'contacts', 2)){
							var html = '';
						...
						if(Engine.Helper.isSet(data,['relations','contacts'])){
							for(var [id, relation] of Object.entries(data.relations.contacts)){
								if(relation.isActive||Engine.Auth.validate('custom', 'contacts_isActive', 1)){
									Engine.Plugins.contacts.Layouts.details.GUI.contact(relation,layout);
								}
							}
						}
					});
					...
					if(Engine.Helper.isSet(data,['relations','contacts'])){
						for(var [id, relation] of Object.entries(data.relations.contacts)){
							if(layout.content.contacts.find('div.contactCard[data-id="'+relation.id+'"]').length <= 0){
								if(relation.isActive||Engine.Auth.validate('custom', 'contacts_isActive', 1)){
									Engine.Plugins.contacts.Layouts.details.GUI.contact(relation,layout);
								}
							}
				...
				Engine.Plugins.contacts.Layouts.details.Events(data,layout);
			...
			GUI:{
				contact:function(dataset,layout,plugin = 'contacts'){
					...
					area.prepend(Engine.Plugins.contacts.Layouts.details.GUI.card(dataset));
					var card = area.find('div.col-sm-12.col-md-6').first();
					if(Engine.Helper.isSet(dataset,['users'])){
						if(Engine.Auth.validate('custom', 'contacts_btn_details', 1)){
							card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'primary',icon:'fas fa-user',action:'details',content:Engine.Contents.Language['Details']}));
						}
					} else {
						if(Engine.Auth.validate('custom', 'contacts_btn_link', 1)){
							card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'teal',icon:'fas fa-link',action:'link',content:Engine.Contents.Language['Add User']}));
						}
					}
					if(Engine.Helper.isSet(dataset,['event_attendances'])){
						if(Engine.Auth.validate('custom', 'contacts_btn_attendance', 1)){
							card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'navy',icon:'fas fa-calendar-check',action:'attendance',content:Engine.Contents.Language['Attendance']}));
						}
					} else {
						if(Engine.Auth.validate('custom', 'contacts_btn_add_attendance', 1)){
							card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'olive',icon:'fas fa-calendar-plus',action:'add',content:Engine.Contents.Language['Add Attendance']}));
						}
					}
					if(Engine.Auth.validate('plugins', 'contacts', 3)){
						card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'warning',icon:'fas fa-edit',action:'edit',content:Engine.Contents.Language['Edit']}));
					}
					if(Engine.Auth.validate('custom', 'contacts_btn_delete', 1)){
						card.find('div.btn-group').append(Engine.Plugins.contacts.Layouts.details.GUI.button(dataset,{id:'id',color:'danger',icon:'fas fa-trash-alt',action:'delete',content:''}));
					}
				},
			...
			Events:function(dataset,layout,options = {},callback = null){
				...
				if(Engine.Auth.validate('plugins', 'contacts', 2)){
					contacts.find('.addContact').off().click(function(){
						Engine.CRUD.create.show({ plugin:'contacts', keys:skeleton, set:{isActive:'true',relationship:url.searchParams.get("p"),link_to:dataset.this.raw.id} },function(created,user){
							if(created){
								user.dom.name = '';
								...
								Engine.Plugins.contacts.Layouts.details.GUI.contact(user.dom,layout);
								Engine.Plugins.contacts.Layouts.details.Events(dataset,layout);
								Engine.Plugins.contacts.Timeline.object(dataset,layout);
							}
						});
					});
				}
					...
					switch($(this).attr('data-action')){
						...
						case"add":
							Engine.Builder.modal($('body'), {
								...
								header.find('button[data-control="update"]').remove();
								body.html('<div class="row"></div>');
								Engine.Builder.input(body.find('div.row'), 'setVows', dataset.setVows,{plugin:'contacts',type:'switch'}, function(input){
						...
						case"delete":
							...
							Engine.CRUD.delete.show({ keys:contact,key:'name', modal:true, plugin:'contacts' },function(user){
								if(contacts.find('[data-id="'+contact.id+'"]').find('.ribbon-wrapper').length > 0 || !Engine.Auth.validate('custom', 'contacts_isActive', 1)){
									contacts.find('[data-id="'+contact.id+'"]').remove();
									layout.timeline.find('[data-type="address-card"][data-id="'+contact.id+'"]').remove();
								}
								if(contact.isActive && Engine.Auth.validate('custom', 'contacts_isActive', 1)){
